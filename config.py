...
class AppSettings(BaseSettings):
    ...
    def _check_system1_not_laya(self) -> "AppSettings":
        ...
        # a stray OPENJEV_SYSTEM1_MODEL in .env, or an OS-level env var left
        # over from a previous run -- all resolve through this same field.
        if self.system1_model == self.laya_model_name:
            raise ValueError(
                f"system1_model and laya_model_name are both '{self.system1_model}'. "
                "system1_model must be a causal-LM checkpoint (e.g. a Qwen2.5 repo id) -- "
                "LayaClassifier._fallback() loads it via AutoModelForCausalLM for the "
                "'Others' branch, which Laya's own repo id can't satisfy. Check for an "
                "OPENJEV_SYSTEM1_MODEL override in .env or your shell environment."
            )
        return self
