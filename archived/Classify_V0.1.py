...
class OpenJevClassifier:
    ...
    def __init__(self, taxonomy: dict, settings: AppSettings, device: str | None = None):
        ...
        self.tokenizer = AutoTokenizer.from_pretrained(settings.system1_model)

        # Tried model-specific tokenizers (LLaMA, Gemma) but they don't work with Qwen2.5-1.5B-Instruct, 
        # so just use the AutoTokenizer for now. 
